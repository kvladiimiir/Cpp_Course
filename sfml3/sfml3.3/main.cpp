...
void updateEyeElements(const sf::Vector2f &mousePosition, Eye &eyeRight, Eye &eyeLeft)
{
    ...
    sf::Vector2f pupilOffsetRight = toEuclidean(20, 40, eyeRight.rotation);
    if ((pow((mousePosition.x - eyeRight.position.x), 2) / pow(pupilOffsetRight.x, 2) + pow((mousePosition.y - eyeRight.position.y), 2) / pow(pupilOffsetRight.y, 2)) > 1.f)
    {
        eyeRight.pupil.setPosition(eyeRight.position + pupilOffsetRight);
    }
    else
    {
        eyeRight.pupil.setPosition(mousePosition);
    }

    eyeLeft.ellipse.setPosition(eyeLeft.position);
    sf::Vector2f pupilOffsetLeft = toEuclidean(20, 40, eyeLeft.rotation);
    if ((pow((mousePosition.x - eyeLeft.position.x), 2) / pow(pupilOffsetLeft.x, 2) + pow((mousePosition.y - eyeLeft.position.y), 2) / pow(pupilOffsetLeft.y, 2)) > 1.f)
    {
        eyeLeft.pupil.setPosition(eyeLeft.position + pupilOffsetLeft);
    }
    else
    {
        eyeLeft.pupil.setPosition(mousePosition);
    }
}
