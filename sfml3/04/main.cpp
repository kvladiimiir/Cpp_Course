...
{
    const sf::Vector2f head0ffset = toEuclidean(40, arrow.rotation);
    arrow.head.setPosition(arrow.position + head0ffset);
    arrow.head.setRotation(toDegrees(arrow.rotation));

    const sf::Vector2f stem0ffset = toEuclidean(-10, arrow.rotation);
